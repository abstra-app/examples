...
def transform_tags(tags):
    ...
    for tag in tags:
        transformed.append(
            {"value": unidecode((tag.lower())), "label": tag.replace("_", " ").capitalize()})
    return transformed
...
def generate_document(file_response, team_member_name, contract_folder, contract_data={}):
    ...
    new_tags_original = [
        tag for tag in tags_original if tag not in list(contract_data.keys()) or contract_data[tag] is None]
...
contract_type = [{'label': 'Individual', 'value': 'individual'},
                 {'label': 'Individual Freelancer',
                     'value': 'individual_freelancer'},
...
def render(partial):
    if len(partial) != 0:
        if partial["existing_contract"] == "New Contract":
            ...
            return Page().read_dropdown("Contract Type", contract_type, key="contract_type")


contract = Page().read_dropdown("Contract Model", ["New Contract", "Existing Contract model"], key="existing_contract")\
                 .reactive(render)\
                 .run()
...
team_info = run_finance('SELECT t.created_at, t.name, t.position, t.email, t.birth_date, t.identification_number, \
                    ...
                    ON t.id = te.team_id LEFT JOIN "team_bank_account" tb ON te.team_id = tb.team_id WHERE t.id = $1', params=[team_id])[0]
...
if contract["existing_contract"] == "New Contract":
    ...
    if contract_type == "individual":
        ...
        contract = open(
            f"contract_models/{contract_type}_contract_ptbr.docx", 'rb').read()
