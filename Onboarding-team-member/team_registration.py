...
company_member = (
    Page()
    .display("Company Data", size="large")
    .read_cnpj(
        "Legal entities number (CNPJ)", required=False, key="legal_entity_number"
    )
    ...
    .read(
        "Company address number (without number)",
        required=False,
        key="company_number_address",
    )
    .read(
        "Company address complement", required=False, key="company_complement_address"
    )
...
run_finance(
    'INSERT INTO "team_bank_account" (name, number, branch_code, team_id) VALUES($1, $2, $3, $4)',
    params=[bank_name, bank_account_number, bank_branch_code, result[0]["id"]],
)

# Insert company data
if name_company is not None:
    run_finance(
        'INSERT INTO "team_entity_informations"(entity_number, name, state, city, address,\
