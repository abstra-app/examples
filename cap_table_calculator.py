...
def extra_fields(data):
    ...
    if not data or not 'dilution_type' in data:
        ...
        return Page()\
            .read_number("How much % was reserved", key="amount", required=False)
...
data_sent = read_multiple_choice("How would you like to enter the investors' data?", [
                                 "Manually", "Via spreadsheet"])

if data_sent == "Manually":
    dilution_schema = ListItemSchema()\
        .read("What is the name of this round?", key="name",max=10,min=1)

    dilutions = read_list(dilution_schema, key="dilutions")
    ...
    for dilution in dilutions:
        ...
        entity_schema = ListItemSchema()\
            .read("Name", key="name")\
            .read_dropdown("What kind of dilution is that?", dilution_types, key="dilution_type")\
            .read_date("When did it happen?", key="date")\
            .reactive(extra_fields)

        dilution['entities'] = Page()\
            .display(f"Members in '{dilution['name']}'")\
            .read_list(entity_schema, key="entities")\
            .run()
else:
    Page()\
        .display("Download the spreadsheet below and fill it with your investors' info:")\
        .display_file('files/investors_data_template.xlsx')\
        .run()
    ...
    # Spreadsheet pre-processing
    import pandas as pd

    data = pd.ExcelFile(rounds)
    sheet_name = data.sheet_names[0]
    df = pd.read_excel(url, sheet_name, header=0, decimal=".")
    ...
    for round_name in rounds_name:
        ...
        dilution['entities'] = {
            'entities': df_round_name.to_dict(orient='records')}
...
for dilution in dilutions:
    ...
    percent_entities = filter(
        lambda entity: entity['dilution_type'] == 'percent', dilution['entities']['entities'])

    for entity in percent_entities:
        percent = entity['amount']/100

        dilution_in_round = dilution_in_round + percent
        add_to_cap_table(entity['name'], dilution['name'],
                         percent, entity['date'])

    cap_entities = filter(
        lambda entity: entity['dilution_type'] == 'cap', dilution['entities']['entities'])
    min_cap = None
    ...
    mfn_entities = filter(
        lambda entity: entity['dilution_type'] == 'mfn', dilution['entities']['entities'])
    if last_min_cap:
        for entity in mfn_entities:
            percent = entity['amount']/last_min_cap
            dilution_in_round = dilution_in_round + percent
            add_to_cap_table(
                entity['name'], dilution['name'], percent, entity['date'])
    ...
    remaining_entities = list(filter(
        lambda entity: entity['dilution_type'] == 'remaining', dilution['entities']['entities']))
...
df = pd.DataFrame.from_dict(cap_table_entries)
df = df.sort_values(by='Signature Date')
df['Signature Date'] = pd.to_datetime(
    df["Signature Date"]).dt.strftime('%Y-%m-%d')
df = df.set_index("Entity")
df.to_csv("/tmp/cap_table.csv", index=False)

Page()\
    .display_pandas(df, full_width=True)\
    .display_file("/tmp/cap_table.csv", download_text="Download your cap table here")\
    .run()